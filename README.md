...
**If you see “Failed to reach Ollama at https://127.0.0.1:8443”:** Restart the proxy after pulling latest changes — it sends **`Access-Control-Allow-Private-Network: true`** so Chrome allows **public HTTPS → 127.0.0.1**. If the app’s origin is not in the built-in allowlist (e.g. a different Vercel preview URL), start the proxy with **`OLLAMA_PROXY_ALLOWED_ORIGINS`** (comma-separated **exact** `Origin` values, e.g. `https://my-preview.vercel.app`). **`Server-side`** FastAPI extraction should use **`OLLAMA_BASE_URL=http://127.0.0.1:11434`** (plain HTTP to Ollama); the HTTPS proxy is aimed at **browser** calls and rejects requests without a permitted `Origin`.
