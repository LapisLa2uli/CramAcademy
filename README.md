...
**HTTPS vs HTTP:** If the app is loaded over **`https://`** (e.g. Vercel), the browser **blocks** `fetch` to **`http://127.0.0.1:11434`** (mixed content). Local extraction with Ollama must run from **`http://localhost:3000`** during development, or you must expose Ollama behind **HTTPS** on localhost (tunnel, or nginx with TLS) so the page can call it without mixed-content blocking.
